...
//       TODO: REMOVE the hashmap and only allow waiting for one kind of PtraceFutureTypes at a
//       time.
...
impl PtraceAsyncRuntime {
    ...
    pub fn unblock_futures(&self, future_type: PtraceFutureTypes, status: PtraceStatus) {
        ...
        self.unblock_future_by_type.borrow_mut().replace(future_type);
    }
...
mod tests {
    ...
    fn test_blocking_on_two_ptrace_futures() {
        ...
        let mut test_future = async {
            runtime.new_ptrace_future(futures::PtraceFutureTypes::WaitForPtraceSyscall).await;
            runtime.new_ptrace_future(futures::PtraceFutureTypes::WaitForSignal).await;
            42
        };
        assert!(matches!(runtime.run_async_step(&mut test_future), Ok(None)));
        assert!(runtime.has_new_blockage());
        ...
        let event3 = futures::PtraceStatus { wait_status: WaitStatus::StillAlive };
    ...
    fn test_compatible_with_futures_lite_zip_in_order() {
        ...
        let event3 = futures::PtraceStatus { wait_status: WaitStatus::Continued(Pid::from_raw(0.into())) };
        runtime.unblock_futures(futures::PtraceFutureTypes::WaitForSignal, event3.clone());
    ...
    fn test_compatible_with_futures_lite_zip_in_reversed_order() {
        ...
        // Unblock the first future
        let event3 = futures::PtraceStatus { wait_status: WaitStatus::Continued(Pid::from_raw(0.into())) };
        runtime.unblock_futures(futures::PtraceFutureTypes::WaitForPtraceSyscall, event3.clone());
        let (output1, output2) = runtime.run_async_step(&mut test_future).unwrap().unwrap();
